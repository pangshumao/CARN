...
class ImagesDataSet(DataSet):
    ...
    def shuffle_images_and_labels(self, images, labels, recon_labels, adjacentMatrix):
        rand_indexes = np.random.permutation(images.shape[0])
        shuffled_images = images[rand_indexes]
        shuffled_labels = labels[rand_indexes]
        shuffled_recon_labels = recon_labels[rand_indexes]
        shuffled_adjacentMatrix = adjacentMatrix[rand_indexes][:, rand_indexes]
        return shuffled_images, shuffled_labels, shuffled_recon_labels, shuffled_adjacentMatrix
        ...
        if normalization_type == 'divide_255':
            ...
            for i in range(images.shape[-1]):
                images[:, :, :, i] = ((images[:, :, :, i] - self.images_means[i]) /
                                      self.images_stds[i])
        elif normalization_type == 'divide_mean':
            for i in range(images.shape[0]):
                for j in range(images.shape[-1]):
                    imMean = np.mean(images[i, :, :, j])
                    images[i, :, :, j] /= imMean
        elif normalization_type == 'subtract_mean':
            for i in range(images.shape[0]):
                for j in range(images.shape[-1]):
                    imMean = np.mean(images[i, :, :, j])
                    imStd = np.std(images[i, :, :, j])
                    images[i, :, :, j] -= imMean
                    images[i, :, :, j] /= (imStd + 1e-10)
